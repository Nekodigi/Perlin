...
void draw(){
  ...
  for(Agent agent:agents){
    ...
    agent.x = lerp(agent.x, width/2, 0.002);
          agent.y = lerp(agent.y, height/2, 0.002);
    if(agent.x <= 0 || agent.x >= width || agent.y <= 0 || agent.y >= height){
      ...
      fill(agent.col);
      noStroke();
      rect(agent.childx, agent.childy, 1, 1);
    ...
    Nagent.childx = lerp(agent.childx, agent.x , 0.001);
    Nagent.childy = lerp(agent.childy, agent.y , 0.001);
